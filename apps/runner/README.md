...
- `RUNTIME_ENABLE_REMOTE_DOCKER` (default: `true`; applies to `executionMode=remote`)
- `RUNTIME_ENABLE_LOCAL_DOCKER` (default: `false`)
- `RUNTIME_LOCAL_DOCKER_IMAGE` (default: `node:20-alpine`)
- `RUNTIME_LOCAL_DOCKER_IMAGE_PROFILES` (default: empty; format `profile=image`, canonical profiles: `default`, `node`, `python`, `rust`; aliases like `js/javascript`, `py`, and `rs` normalize to canonical profiles)
